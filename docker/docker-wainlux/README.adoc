...
Compose files live in `docker/` (parent directory), not in this folder.
...
* `ghcr.io/<owner>/<repo>:armv6-latest` (main branch)
* `ghcr.io/<owner>/<repo>:armv6-sha-<commit>`

NOTE: ARMv6 prebuilt image is laser API + driver only.
MCP is decoupled and runs from `docker-mcp/` as a separate container.
Build runs import smoke checks for runtime dependencies so broken wheel/ABI combinations
fail in CI instead of failing at container runtime.
...
docker pull ghcr.io/<owner>/<repo>:armv6-latest
----

To run with compose from a prebuilt image, set `image:` for the service in `compose.yaml`
to `ghcr.io/<owner>/<repo>:armv6-latest` and remove/ignore the `build:` entry.
...
* *Volume*: `./docker-wainlux/data` (from `docker/` compose root)
