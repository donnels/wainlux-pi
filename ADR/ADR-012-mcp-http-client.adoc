...
Accepted (Updated 2026-02-13: MCP runtime decoupled from Pi)
...
Pi Zero W is too old for reliable builds of modern MCP Python dependencies. This is a deployment constraint, not a protocol constraint.
...
* MCP runs in its own container on a modern 64-bit host by default.
* Pi runs Flask API + K6 driver container only.
* Running MCP on Pi is optional and not part of the default deployment.
...
* MCP and laser API can be upgraded independently.
* ARMv6 dependency failures in MCP no longer block laser deployment.
...
* Default Pi deployment no longer includes on-box MCP service.
