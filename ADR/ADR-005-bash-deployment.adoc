...
Single Pi Zero W. One required Docker container on Pi (laser API + driver).
MCP may run in a separate container on another host.
...
One Pi. One required container. One person. Bash wins.
MCP is optional and decoupled.
