...
Each data chunk (opcode `0x22`) requires ACK `0x09` before sending next chunk. 100ms pause after motion commands.
...
=== Raster and Vector (mixed supported)

- Burn engine is raster; vectors are sent as point coordinates. Device rasterizes both.
- Mixed jobs are supported: raster bytes first, then optional vector point pairs (little-endian x,y) appended in the same DATA stream; one JOB_HEADER covers both.
- Vector fields in the header (vector size/power/depth/count) must be set when points are present; otherwise zeroed for raster-only.
- Why vector: fewer bytes for sparse outlines or fills with separate power/depth; raster remains better for dense fills/photos.
...
* Parameters include width, height, position, speed/power, vector metadata, and payload size.
* All multi-byte fields are *big-endian* (MSB first).
* Raster width/height are raw pixel dimensions (no scaling applied).
* Block count (bytes 3-4) observed as `(payload_bytes + 0x21 + vector_bytes) / 0xFFE + 1`.
* Total size (bytes 20-23) observed as `payload_bytes + 0x21` (add vector bytes when present).
* Vector fields (w/h/power/depth/count) are set when point data is present; zeroed for raster-only jobs.
* Center offsets use +67 formula (see <<Job Header Parameters>>).
* Work area observed: 80x80mm, resolution 0.05 mm/pixel ⇒ max 1600 x 1600px.
...
Chunk size: 0x76c (1900) bytes of payload.

Packet layout (per chunk) (raster bytes, then optional vector point bytes when present):
...
[WARNING]
====
**Device rejects fully blank DATA chunks (error 0x08).**
...
Raw protocol check (22-Feb-2026, firmware `v9.4.1`):

* All-zero DATA payload (`0x22`, len `0x004c`, 72 bytes all `00`) -> response `0x08` (error)
* Same packet with one non-zero byte in payload (example first byte `FF`) -> response `0x09` (ACK)
* In repeated tests, blank DATA could return multiple error bytes (`08 08 08 08`)

**Workaround:** Ensure no transmitted DATA chunk is entirely zero.

* Crop/trim white margins before packing (top/bottom/left/right)
* Preflight packed bytes and detect `all(chunk_byte == 0)` per transmitted chunk
* Reposition using JOB_HEADER center (`center_x`, `center_y`) after cropping
...
11-byte packet. Observed sequence: sent twice after job data with ~200 ms then ~500 ms spacing:
