...
Flask (port 8080) owns serial/device access.
MCP (port 8081) is an HTTP client adapter to Flask.
...
Flask and MCP run as separate services in compose:

[source,bash]
----
docker compose up -d
docker compose logs -f wainlux-mcp
----
